...
kernel void stretch(
       ...
       int redOffset,		//this iter start from redwalkers+redOffset
       int blkOffset,		//this iter start from blkwalkers+blkOffset
...
//	float *xj = (float *)blkwalkers+blkOffset+j*dim;
	...
	int i=0;
	for(i=0;i<dim;i++){
		*(Y+k*dim+i)=*(blkwalkers+blkOffset+j*dim+i)+
				z*(*(redwalkers+redOffset+k*dim+i)-
				*(blkwalkers+blkOffset+j*dim+i));
	}
		
	//get acceptance rate
	float q= pow(z,dim-1)*probability(Y+k*dim,dim)/
				probability(redwalkers+redOffset+k*dim,dim);
...
//	float *xk2 = (float*)redwalkers+redOffset+(k+walkerN/2)*dim;
	if (randnum.s2<=q){
		//write y to redwalkers end
	   	for(i=0;i<dim;i++){
//			xk2[i]=y[i];
			*(redwalkers+redOffset+k*dim+walkerN/2*dim+i)
			=*(Y+k*dim+i);
